    ...
    case "$PKG_VERSION" in
        *-rc.*)
            ...
            "${VENV_DIR}/bin/pip" install --quiet \
                "image-generation-mcp${EXTRAS} @ https://github.com/pvliesdonk/image-generation-mcp/releases/download/v${PKG_VERSION}/image_generation_mcp-${canonical}-py3-none-any.whl"
            ;;
        *)
            "${VENV_DIR}/bin/pip" install --quiet "image-generation-mcp${EXTRAS}==${PKG_VERSION}"
            ;;
    esac
else
    "${VENV_DIR}/bin/pip" install --quiet "image-generation-mcp${EXTRAS}"
fi
