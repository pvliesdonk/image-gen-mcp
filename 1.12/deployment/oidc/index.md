...
The mode is auto-detected when `AUTH_MODE` is not set: `oidc-proxy` when client credentials are present, `remote` otherwise. For an overview of all authentication modes (bearer token, OIDC, no auth), see the [Authentication guide](https://pvliesdonk.github.io/image-generation-mcp/1.12/guides/authentication/index.md).
...
- **OIDCProxy mode** verifies the `id_token` (always a standard JWT) instead of the access token, so opaque tokens work without extra configuration. However, oidc-proxy is subject to the [session lifetime limitation](https://pvliesdonk.github.io/image-generation-mcp/1.12/guides/authentication/#known-limitations-oidc-session-lifetime).
...
Use oidc-proxy only when remote mode is not viable (such as when your IdP cannot issue JWT access tokens). Be aware of the [session lifetime limitation](https://pvliesdonk.github.io/image-generation-mcp/1.12/guides/authentication/#known-limitations-oidc-session-lifetime).
...
1. Client sends requests to the MCP server with the JWT token
1. Server validates the token locally using the IdP's JWKS keys
1. No upstream token storage or re-validation
...
1. Server exchanges the code for tokens and issues its own proxy JWT
1. Subsequent requests include the proxy JWT

Session lifetime

OIDCProxy re-validates upstream tokens on every request. When the upstream token expires (typically 1 h), sessions die even though the proxy JWT is still valid. See [Known Limitations](https://pvliesdonk.github.io/image-generation-mcp/1.12/guides/authentication/#known-limitations-oidc-session-lifetime).
