...
`src/image_generation_mcp/config.py` between the `CONFIG-FIELDS-START` /
...
declares `IMAGE_GENERATION_MCP_SERVER_NAME` and `IMAGE_GENERATION_MCP_INSTRUCTIONS`
...
1. Move each hand-written row's content into the corresponding
   ...
   and `CONFIG-FIELDS-END` sentinels in `src/image_generation_mcp/config.py`:
...
auth vars such as `IMAGE_GENERATION_MCP_OIDC_CLIENT_ID` and
`IMAGE_GENERATION_MCP_BEARER_TOKEN` now appear only in the `oci` package's
...
rich-logging switches, together with `IMAGE_GENERATION_MCP_SERVER_NAME`,
`IMAGE_GENERATION_MCP_INSTRUCTIONS`, and `IMAGE_GENERATION_MCP_KV_STORE_URL`.
