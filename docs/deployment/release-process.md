...
requirement pins one or you pass `--pre`, so `pip install image-generation-mcp`
and `uv add image-generation-mcp` still resolve to the newest stable. Ask for a
candidate by name to get one:

```bash
pip install image-generation-mcp==X.Y.ZrcN
```
...
carrying the code. Its manifest launches `uvx --from image-generation-mcp[all]==<version>`,
...
Every release attaches `image-generation-mcp-plugin-<version>.zip`, and every
merge to `main` produces the same archive as the `plugin-zip-edge` workflow
artifact. Load one without installing anything:

```bash
claude --plugin-url https://github.com/pvliesdonk/image-generation-mcp/releases/download/vX.Y.Z-rc.N/image-generation-mcp-plugin-X.Y.Z-rc.N.zip
```
...
```bash
claude --plugin-url <url> plugin details image-generation-mcp
```
...
differently: it is a thin pointer that installs `image-generation-mcp` from PyPI,
...
repository, so a cross-repo epic stays visible. The warning is advisory;
a deliberate cut remains possible.
