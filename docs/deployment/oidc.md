...
Use oidc-proxy only when remote mode is not viable (such as when your IdP cannot issue JWT access tokens). Be aware of the [session lifetime limitation](../guides/authentication.md#known-limitations-oidc-session-lifetime).
...
!!! warning "Session lifetime"
    OIDCProxy re-validates upstream tokens on every request. When the upstream token expires (typically 1 h), sessions die even though the proxy JWT is still valid. See [Known Limitations](../guides/authentication.md#known-limitations-oidc-session-lifetime).
