"""Guard for the public import surface of the `image_generation_mcp` package root.
...
_HEADER = """\
# Public import surface of the `image_generation_mcp` package root — one name
...
module = importlib.import_module("image_generation_mcp")
...
def test_snapshot_file_exists() -> None:
    ...
    assert SNAPSHOT.is_file(), (
        ...
        f"import surface of `image_generation_mcp`. Generate it with:\n"
        f"    {UPDATE_COMMAND}\n"
...
def test_public_import_surface_matches_snapshot() -> None:
    ...
    if removed:
        problems.append(
            ...
            f"`image_generation_mcp`: {', '.join(removed)}\n"
    ...
    if added:
        problems.append(
            ...
            f"`image_generation_mcp`: {', '.join(added)}\n"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Public import-surface snapshot tool for `image_generation_mcp`."
    )
    parser.add_argument(
        "--update",
        action="store_true",
