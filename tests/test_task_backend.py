...
`IMAGE_GENERATION_MCP_TASKS_URL` is documented automatically — the config-surface
...
_DERIVED_QUEUE_NAME = "image-generation-mcp"
"""What pvl-core derives from `IMAGE_GENERATION_MCP` — lower-cased, `_` to `-`.
...
def test_tasks_url_reaches_the_backend() -> None:
    """An explicit `IMAGE_GENERATION_MCP_TASKS_URL` selects the Docket backend."""
